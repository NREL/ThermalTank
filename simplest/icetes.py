...
class UsrDefPlntCmpSim(EnergyPlusPlugin):
    ...
    def get_userdef_handles(self, state):
    ...
    def on_begin_timestep_before_predictor(self, state) -> int:
        ...
        if ((chrg_sch == 0) or ((chrg_sch == 1) and (self.tank_is_full))):
            self.tank_branch.simulate(self.t_in, self.mdot_in, 20, t_set_icetank, 0, datetime, timestep)
            self.api.exchange.set_actuator_value(state, self.t_set_chiller_hndl, 6.7)
            self.t_out = self.t_in

        # else charge or discharge tank
        else:
            self.tank_branch.simulate(self.t_in, self.mdot_in, 20, t_set_icetank, chrg_sch, datetime, timestep)
    ...
    def on_user_defined_component_model(self, state) -> int:
        ...
        if self.need_to_get_userdef_handles:
            self.get_userdef_handles(state)
            self.glycol = self.api.functional.glycol(state, u'water')
        ...
        self.t_in = self.api.exchange.get_internal_variable_value(state, self.t_in_hndl)
        self.mdot_in = self.api.exchange.get_internal_variable_value(state, self.mdot_in_hndl)
        self.cp_in = self.api.exchange.get_internal_variable_value(state, self.cp_in_hndl)
        self.load_in = self.api.exchange.get_internal_variable_value(state, self.load_in_hndl)
        ...
        self.api.exchange.set_actuator_value(state, self.mdot_out_hndl, self.mdot_out)
        self.api.exchange.set_actuator_value(state, self.t_out_hndl, self.t_out)
