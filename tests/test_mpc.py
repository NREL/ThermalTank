...
def test_hourly_opt():
    ...
    start_ts = 24 * 240 * math.ceil(n/24)

    # construct the tank and the building
    data = copy.deepcopy(tank_data)
    data['initial_temperature'] = df_states['tank_temp'].values[start_ts - 1]
    data['latent_state_of_charge'] = df_states['soc'].values[start_ts - 1]
    tank = IceTank(data)
    m, tank = full_model(tank, simple=False, calc_mode=True, n=n - 1, timestep=60)
    building_loop_model(m)
    
    # initialize the TES and chiller's temp at time = -1 and load up the initial state variables into MPC
    init_temp(m, data['initial_temperature'], data['latent_state_of_charge'], -1)
    obj = populate_values(m, start_ts, df_states=df_states)

    df_states_day = df_states[df_states.index.isin(range(start_ts, start_ts + n))]
    bldg_only_elec = (df_states_day['bldg_elec_load'] - df_states_day['chiller_power_new']).values
    old_max_load = max(df_states_day['bldg_elec_load'])

    for i in m.horiz_t:
        m.cond_inlet[i].set_value(df_states_day.iloc[i]['cond_inlet'] + 273.15)
        ch_val = value(chiller_a[0] * m.cond_inlet[i] + chiller_a[1] * m.chiller_delta_T[i] + chiller_b)
        m.chiller_power[i].set_value(max(ch_val, 0))
    ...
    solver.options['max_iter'] = 5000
    solve_log = idaeslog.getInitLogger("infeasibility", idaeslog.INFO, tag="properties")
    log_infeasible_constraints(m, logger=solve_log, tol=10, log_expression=True, log_variables=True)

    # sim_tanktemp, sim_outlettemp, sim_branchtemp, sim_tankflowfrac = run_tank_model(m, data, True)

    # no integer variables: fix to phase change / latent discharge and charging / discharging mode
    m.tank_temp_b1.fix()
    m.tank_temp_b2.fix()
    m.is_charging.fix()

    # init tank and then unfix tank temperatures
    m.obj = Objective(expr=obj)
    m.tank_temp_d1.unfix()
    m.tank_temp_d2.unfix()
    res = solver.solve(m, tee=True)

    # minimize the max load
    m.max_outlet_temp = Constraint(m.horiz_t, rule=lambda m, i: m.outlet_settemp[i] <= 7 + 273.15)
    m.bldg_elec_load = Expression(m.horiz_t, rule=lambda m, i: m.chiller_power[i] + bldg_only_elec[i])
    m.peak = Var(domain=NonNegativeReals, initialize=max(value(m.bldg_elec_load[i]) for i in m.horiz_t))
    m.peak_demand = Constraint(m.horiz_t, rule=lambda m, i: m.peak >= m.bldg_elec_load[i])

    m.del_component("obj")
    m.obj = Objective(expr=m.peak * 1e3)
    solve_log = idaeslog.getInitLogger("infeasibility", idaeslog.INFO, tag="properties")
    ...
    if res.Solver.Status != 'ok':
        solve_log = idaeslog.getInitLogger("infeasibility", idaeslog.INFO, tag="properties")
        log_infeasible_constraints(m, logger=solve_log, tol=1e-3, log_expression=True, log_variables=True)

    new_max_load = max(value(m.bldg_elec_load[i]) for i in m.horiz_t)
    plot_modified_operation(m, df_states, start_ts, n)

    assert new_max_load < old_max_load
    print("time", default_timer() - solver_start)

    # now do integer opt
    m.tank_temp_b1.unfix()
    m.tank_temp_b2.unfix()
    m.is_charging.unfix()

    solver = SolverFactory("bonmin")
    solver.options['tol'] = 1e-5
    solver.options['max_iter'] = 1500
    solver.options['bonmin.time_limit'] = 60 * 10
    solver.options['bonmin.iteration_limit'] = 10000
    res = solver.solve(m, tee=True)
    new_new_max_load = max(value(m.bldg_elec_load[i]) for i in m.horiz_t)
    plot_modified_operation(m, df_states, start_ts, n)
    assert new_new_max_load <= new_max_load
